#' fill or set zero except for the first positive values
#'
#' Fill or set zero except for the first positive values of a numerical matrix
#' by rownames
#'
...
#'
#' # set zero except for the first positive values of a numerical matrix by rownames
#' \donttest{x <- matrix(c(-1, 2, 0, -2, -1, 5, 2, -2, 3), nrow = 3)
#' set_rownames(x, c(1, 1, 2))
...
#'
#' Fill with one before the first one appears in each column and by rownames of
#' the binary matrix.
#'
...
#'
#' @examples
#' # fill with one before the first one appears
#' \donttest{x <- matrix(c(0, 1, 0, 0, 0, 1, 1, 0, 1), nrow = 3)
#' set_rownames(x, c(1, 1, 2))
#' fill_one_before_first_one(x)}
#'
#' # Set one before the first one appears
#' \donttest{x <- matrix(c(0, 1, 0, 0, 0, 1, 1, 0, 1), nrow = 3)
#' set_rownames(x, c(1, 1, 2))
#' fill_one_before_first_one(x)}
#'
